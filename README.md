...
5. 选择清晰度：
   - `最高可用画质`：选择当前账号和视频源能拿到的最高可用画质。YouTube 如果有 2K/4K，会优先拿 2K/4K；必要时自动转成 Mac 可播放格式。
   - `优先 60 帧`：平台和账号支持时优先下载高帧率版本。
   - `2160P / 4K`、`1440P / 2K`、`1080P`、`720P`、`480P`、`360P`：限制最高视频高度。
6. 保持默认保存目录，或者输入你自己的本地目录。这个目录会被记住，下次重启工具后仍然使用。
7. 点击 `检测链接` 检查链接。
8. 点击 `开始下载`。
9. 如果任务长时间没有进展，可以点击 `停止任务`。它会停止当前下载或转码任务，不需要用 `Ctrl+C` 退出整个服务。

也可以在页面的 `压缩本地视频` 区域选择电脑里的 MP4、MOV、M4V、MKV 或 WEBM 文件，并选择约 50 MB、25 MB 或 15 MB 的目标大小。压缩结果会保存到当前保存目录，原始文件不会被修改。上传到 GPT 时推荐先选择约 25 MB。
...
- 工具不会保存用户名或密码。
- YouTube 的 `最高可用画质` 会优先选择真实最高分辨率，不再为了 H.264 兼容性把 2K/4K 降到 1080P。如果下载到 AV1/VP9 等 Mac 可能只播声音的格式，工具会自动转成 Mac 可播放的 H.264 MP4。
- YouTube 4K 常见流程是先下载最高画质的视频流，先用更稳定的 MKV 中间容器合并，再转换成 Mac 可播放 MP4。这个阶段页面会显示“正在转换为 Mac 可播放 MP4”和转码百分比；如果转换阶段连续 5 分钟没有任何进度，工具会自动停止并提示错误，避免一直卡住。
